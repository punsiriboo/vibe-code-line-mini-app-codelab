...
title: Building LINE MINI App for restaurant reservations using Google AI Studio
...
Feedback Link: https://forms.gle/rdE9LGrn96VoeKS4A

# Building LINE MINI App for restaurant reservations using Google AI Studio
...
Codelab นี้จะพาคุณไป Prompt สร้างระบบจองร้านอาหารบน LINE MINI App
...
- สร้างเว็บจองร้านอาหารด้วย Prompt ใน [Google AI Studio](https://ai.studio/build/)
...
- ตั้งค่า Service Message Template และส่งข้อความยืนยันการจองโต๊ะอาหาร
...
### Vibe Coding คืออะไร?
Vibe Coding (ไวบ์ โค้ดดิ้ง) คือแนวทางการพัฒนาแอปพลิเคชันโดยอธิบาย "สิ่งที่ต้องการ" ด้วยภาษาธรรมชาติ (Prompt) แล้วให้ AI สร้างเว็บ UI, Logic และการเชื่อมต่อระบบให้ โดยที่เราไม่ต้องเริ่มจากการเขียนโค้ดทีละบรรทัด
...
ปัจจุบันมีเครื่องมือ Vibe Coding หลายตัวที่ได้รับความนิยม เช่น [Google AI Studio](https://ai.studio/build/) สำหรับสร้าง Full-stack App จาก Prompt และ Deploy ได้ทันที, [Google Antigravity](https://antigravity.google/) เป็น agentic development platform ที่ช่วยสั่งงาน AI agents สร้างและทดสอบแอปใน Editor, Terminal และ Browser, [Cursor](https://cursor.com/) เป็น AI-powered IDE ช่วยเขียนและแก้โค้ดในโปรเจกต์จริง, [Claude Code](https://docs.anthropic.com/en/docs/claude-code) ทำงานเป็น AI Agent ใน Terminal 
...
ในช่วงนี้คุณจะใช้ Google AI Studio สร้างเว็บจองร้านอาหารด้วยการ Vibe Coding ซึ่งจะเป็นการสั่งผ่าน Prompt ที่ทำให้เราไม่ต้องเขียนโค้ดเอง
...
<aside class="positive">
<strong>Tip:</strong> ถ้า UI ยังไม่ตรงใจ ให้ใช้ <strong>Annotation Mode</strong> วาดวงรอบส่วนที่ต้องการแก้ แล้ว Prompt เพิ่ม เช่น "เปลี่ยนชื่อร้านเป็น The Green Table" หรือ "เพิ่มโลโก้ร้านด้านบน"
</aside>
...
![LINE User Profile](img/4.2.png)

LINE MINI App สามารถเชื่อมต่อดึงข้อมูล [LINE User Profile](https://developers.line.biz/en/reference/liff/#get-profile) ผู้ใช้ที่ Login ด้วย LINE ได้
...
![External Browser](img/4.3.png)

LINE MINI App สามารถเปิดใช้งานได้ทั้งภายในแอป LINE บนสมาร์ทโฟน และผ่าน [External Browser](https://developers.line.biz/en/docs/line-mini-app/develop/external-browser/) เช่น Chrome หรือ Safari โดยช่วยให้ผู้พัฒนาไม่ต้องสร้างระบบ Authentication หรือระบบจัดการบัญชีผู้ใช้งานขึ้นมาเอง ลดทั้งเวลาและความซับซ้อนในการพัฒนา
...
#### 3. ส่งข้อความให้เพื่อนหรือกลุ่มด้วย Share Target Picker
![Share Target Picker](img/4.4.png)
[Share Target Picker](https://developers.line.biz/en/reference/liff/#share-target-picker) เป็นฟีเจอร์ของ LIFF ที่ให้ผู้ใช้เลือกเพื่อนหรือกลุ่มใน LINE เพื่อส่งข้อความกลับไปยังห้องแชทของผู้ใช้ได้ทั้งในรูปแบบของ Text Message และ Flex Message ซึ่งใน Codelab นี้จะเป็นการชวนเพื่อนมาทานอาหารด้วยกัน


#### 4. ส่งข้อความแจ้งเตือนด้วย Service Message
![Service Message](img/4.5.png)
[Service Messages](https://developers.line.biz/en/docs/line-mini-app/develop/service-messages/) เป็นฟีเจอร์หนึ่งใน LINE MINI App ที่ให้ผู้ให้บริการส่งข้อความแจ้งเตือนไปยังผู้ใช้งาน LINE MINI App ได้โดยไม่มีค่าบริการ และไม่จำเป็นต้องเพิ่ม LINE OA ใดๆ เช่น เมื่อลูกค้าเข้ามาจองโต๊ะอาหาร ทางร้านก็สามารถส่งข้อความยืนยันการจอง หรือส่งข้อความแจ้งเตือนล่วงหน้าก่อนถึงวันที่จะเข้ามาใช้บริการ

ตัวอย่าง Use Case สำหรับจองร้านอาหาร เช่น ยืนยันการจองโต๊ะสำเร็จ หรือแจ้งเตือนก่อนถึงเวลารับประทาน

## สร้าง Provider และ Channel
Duration: 0:12:00

ก่อนแปลงเว็บเป็น LINE MINI App คุณต้องมี Provider และ LINE MINI App Channel ใน [LINE Developers Console](https://developers.line.biz/console/) ก่อน

### สมัครเป็น LINE Developer

จุดเริ่มต้นสำหรับการพัฒนาแอปพลิเคชันต่างๆ บนแพลตฟอร์มของ LINE คือ คุณจะต้องสมัครเป็น LINE Developer ก่อน
...
Provider คือ superset ของแอปทั้งหลายที่เราจะพัฒนาขึ้น รวมถึง LINE MINI App ด้วย โดยการสร้าง Provider เพียงแค่ระบุชื่อ ซึ่งอาจตั้งเป็นชื่อตัวเอง ชื่อบริษัท ชื่อทีม หรือชื่อกลุ่มก็ได้
...
Requirements
- Integrate LIFF and LINE Login into the existing application using package @line/liff
- Add Login with LINE and Logout button under the hero
...
- Add a LINE User Profile Card at the top of the reservation page after login using liff.getProfile().
- Display the user’s LINE profile picture, LINE ID, display name, and status message if available.
- Pre-fill the Customer Name field with the LINE user’s display name, while keeping the field editable by the user
...
หลังส่ง Prompt หาก Google AI Studio แสดงหน้าต่าง **Enter your environment variable to continue** ให้กรอก **LIFF ID** จากหัวข้อ **ตั้งค่า LINE MINI App** อีกครั้ง แล้วกด **Apply**
...
![ปุ่ม Invite Friends](img/8.2.png)
...
![Flex Message คำเชิญจองโต๊ะ](img/8.3.png)
...
<aside class="negative">
<strong>Important:</strong> LINE MINI App ไม่อนุญาตให้ส่งข้อความโฆษณาหรือโปรโมชันต่างๆ ได้ <a href="https://developers.line.biz/en/docs/line-mini-app/service/service-operation/#conditions-for-service-messages">รายละเอียดเพิ่มเติม</a>
</aside>
...
1. Client Side ส่ง LIFF Access Token และข้อมูลการจองไปยัง Server Side <br/>
...
2. Issue Channel Access Token<br/>
...
1. เปิด LINE MINI App, กรอกข้อมูลจองโต๊ะ แล้วกด **Reserve Now**
...
ยินดีด้วย! ถึงตรงนี้คุณก็มี LINE MINI App ตัวแรกเป็นของคุณเองแล้ว
...
- [Feedback form](https://forms.gle/rdE9LGrn96VoeKS4A)
